    ...
    def fuse_lora_to_base_model(self, model: torch.nn.Module, state_dict, alpha=1.0):
        ...
        for name, module in model.named_modules():
            if name in lora_layer_names:
                ...
                # Under ZeRO-3, a module's parameter is sharded even before
                # Accelerator.prepare().  A local shard can have shape 0 in
                # one dimension, so fusing against module.state_dict() gives
                # misleading shape errors (for example 0 vs 5376).  Gather
                # only this layer, apply the update, and let ZeRO-3 scatter it
                # back.  The regular path is unchanged for non-sharded models.
                base_weight = getattr(module, "weight", None)
                is_zero3_param = base_weight is not None and hasattr(base_weight, "ds_id") and base_weight.ds_id is not None
                if is_zero3_param:
                    try:
                        import deepspeed
                        # CPU ZeRO-3 initialization uses ``remote_device=cpu``.
                        # ``modifier_rank=0`` would force DeepSpeed to broadcast
                        # from CUDA on exit, which is invalid before the model is
                        # moved to the accelerator.  Gather on every rank and
                        # apply the deterministic adapter update locally instead.
                        gather_context = deepspeed.zero.GatheredParameters(
                            [base_weight], modifier_rank=None
                        )
                    except (ImportError, AttributeError):
                        gather_context = contextlib.nullcontext()
                    with gather_context:
                        target_device = base_weight.device
                        base_weight.data = base_weight.data.to(
                            device=target_device, dtype=self.torch_dtype
                        )
                        base_weight.data.add_(weight_lora.to(target_device))
                else:
