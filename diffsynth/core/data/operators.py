...
class FrameSamplerByRateMixin:
    ...
    def get_total_frames(self, reader, meta_data=None):
        if hasattr(reader, "count_frames"):
            return int(reader.count_frames())
        if hasattr(reader, "get_length"):
        ...
        if duration is not None:
            return int(math.floor(float(duration) * fps))
        return None

    def get_available_num_frames(self, reader):
        if not self.fix_frame_rate:
            return self.get_total_frames(
                reader, reader.get_meta_data(index=...)
            ) or 0
        meta_data = reader.get_meta_data(index=...)
        total_original_frames = self.get_total_frames(reader, meta_data)
...
class LoadVideo(DataProcessingOperator, FrameSamplerByRateMixin):
    ...
    def __call__(self, data: str):
        ...
        raw_frame_rate = reader.get_meta_data(index=...)['fps']
        num_frames = self.get_num_frames(reader)
        meta_data = reader.get_meta_data(index=...)
        total_raw_frames = self.get_total_frames(reader, meta_data)
        if total_raw_frames is None:
            total_raw_frames = int(math.floor(float(meta_data["duration"]) * raw_frame_rate))
        frames = []
        ...
        try:
            ...
            try:
                ...
                audio, sample_rate = sf.read(
                    data, dtype="float32", always_2d=True
                )
