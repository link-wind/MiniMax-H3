...
class LoRAHotLoadMixin:
    # Cap on tokens processed per LoRA matmul chunk. Segmenting the token
    # dimension lowers the peak intermediate activation of
    # x @ A.T @ B.T for long sequences (long masked-AV windows), which
    # otherwise drives single-GPU CUDA OOM. The chunked result is exactly
    # equivalent to a single matmul (each token is independent).
    lora_max_tokens_per_chunk = 262144
    ...
    def _lora_matmul(self, x, lora_A, lora_B):
        # x @ A.T @ B.T computed in token chunks for a lower peak footprint.
        # Only the trailing (feature) dim must be the matmul input dim; any
        # leading dims are flattened and restored so segmentation is layout-agnostic.
        A = lora_A.T.to(device=x.device, dtype=x.dtype)  # [in, rank]
        B = lora_B.T.to(device=x.device, dtype=x.dtype)  # [rank, out]
        out_dim = B.shape[-1]                            # out dim from lora_B.T
        flat = x.reshape(-1, x.shape[-1])                # [tokens, in]
        n_tokens = flat.shape[0]
        chunk = self.lora_max_tokens_per_chunk
        result = (flat @ A @ B) if n_tokens <= chunk else torch.cat(
            [c @ A @ B for c in torch.split(flat, chunk, dim=0)], dim=0
        )
        # Output feature dim is the LoRA output dim, not x's last dim.
        return result.reshape(*x.shape[:-1], out_dim)

    def lora_forward(self, x, out):
        if self.lora_merger is None:
            for lora_A, lora_B in zip(self.lora_A_weights, self.lora_B_weights):
                out = out + self._lora_matmul(x, lora_A, lora_B)
        else:
            lora_output = []
            for lora_A, lora_B in zip(self.lora_A_weights, self.lora_B_weights):
                lora_output.append(self._lora_matmul(x, lora_A, lora_B))
