...
def packed_seq_params_to_cp_metadata(packed_seq_params, seq_len=None):
    cu_seqlens = packed_seq_params["cu_seqlens_q"]
    return PackedAttentionMetadata.from_cu_seqlens(
        cu_seqlens.tolist(), seq_len=seq_len
    )


def h3_shard_packed_fields(
    *,
    local_start,
    local_end,
    inverse_indices,
    ...
    def local_positions(positions):
        ...
        return positions[mask] - local_start

    result = {
        "inverse_indices": inverse_indices[local_start:local_end],
        "token_tags": token_tags[local_start:local_end],
        "img_position_ids": img_position_ids[:, local_start:local_end],
        "img_pos": local_positions(img_pos),
        "audio_pos": local_positions(audio_pos),
        "text_pos": local_positions(text_pos),
        "infer_out_pos": local_positions(infer_out_pos),
    }
    if update_mask is not None:
        result["update_mask"] = update_mask[local_start:local_end]
    if update_audio_mask is not None:
        result["update_audio_mask"] = update_audio_mask[local_start:local_end]
    return result


def h3_local_target_rows(
    training_target_video: torch.Tensor,
    training_target_audio: torch.Tensor,
    packed,
    cp_rank: int,
    cp_world_size: int,
    *,
    cond_rows_count: int,
    ref_audio_rows_count: int,
) -> tuple[torch.Tensor, torch.Tensor]:
    """Return local target patch rows for CP-sharded H3 loss."""
    spans = split_sequence_indices(int(packed["seq_len"]), cp_world_size)
    local_start, local_end = spans[cp_rank]

    video_rows = patchify_video(training_target_video)
    audio_rows = pack_audio(training_target_audio)

    img_pos = packed["img_pos"].view(-1).to(torch.long)
    audio_pos = packed["audio_pos"].view(-1).to(torch.long)
    target_img_pos = img_pos[cond_rows_count:]
    target_audio_pos = audio_pos[ref_audio_rows_count:]
    video_mask = (target_img_pos >= local_start) & (
        target_img_pos < local_end
    )
    audio_mask = (target_audio_pos >= local_start) & (
        target_audio_pos < local_end
    )
    return video_rows[video_mask], audio_rows[audio_mask]
...
class MiniMaxH3Attention(nn.Module):
    ...
    def forward(
        ...
        *,
        ...
        if cp_world_size is not None and cp_world_size > 1 and cp_metadata is not None:
            if cp_group is not None:
                ...
                out = logical_ring_varlen_attention_all(
                    q,
                    k,
                    v,
                    cp_metadata,
...
class MiniMaxH3DiT(nn.Module):
    ...
    def _embed(self, *, x, audio_x, text_embeddings_selected, unique_timesteps, img_pos, audio_pos, text_pos, text_embed_select, refiner_cu_seqlens, refiner_max_seqlen, seq_len, device, mem_pos=None):
        ...
        # Memory rows carry real video latents, so they reuse the video
        # projection.  Sharing it (rather than adding a separate memory
        # projection) keeps the memory block in the same embedding space as the
        # window it conditions, and adds no parameters.
        #
        # The projection is invoked *unconditionally*, even when this rank owns
        # no memory row or the sample carries no slot at all.  Under ZeRO-3 with
        # ``stage3_param_persistence_threshold=0`` every module call is a
        # parameter gather, i.e. a collective, so a data-dependent call site
        # makes the collective sequence differ between ranks and hangs the run
        # instead of raising.  Only the ``index_add_`` below stays conditional:
        # it touches no parameters.  An empty slot therefore still means "this
        # sample gets no memory conditioning".
        mem_rows = (
            x.view(-1, x.shape[-1]).index_select(0, mem_pos).to(dtype)
            if mem_pos is not None and mem_pos.numel()
            else x.view(-1, x.shape[-1])[:0].to(dtype)
        )
        mem_embed = self.video_patch_proj(mem_rows)
        ...
        if text_embed_select is not None:
            text_embed = text_embed.index_select(
                0, text_embed_select.to(device=device)
            )
        ...
        if mem_pos is not None and mem_pos.numel():
            embeddings.index_add_(0, mem_pos, mem_embed.to(dtype)[: mem_pos.shape[0]])
    ...
    def forward(
        ...
        # Optional clean memory block: extra video rows that condition the
        # window without ever being prediction targets (they are not in
        # ``img_pos``, so the loss cannot see them).
        mem_pos = None
        if mem_pos_info is not None:
            mem_pos = mem_pos_info["position_ids"].view(-1).to(torch.long)
        ...
        local_seq_len = int(x.shape[1])
        device = x.device

        use_local_cp = (
            cp_world_size is not None
            and cp_world_size > 1
            and cp_group is not None
        )
        cp_enabled = cp_world_size is not None and cp_world_size > 1
        if use_local_cp:
            global_seq_len = int(packed_seq_params.get("seq_len", local_seq_len))
            cp_metadata = PackedAttentionMetadata.from_cu_seqlens(
                cu_seqlens.tolist(), global_seq_len
            )
            shard = PackedShardMetadata(
                cp_metadata, rank=cp_rank, world_size=cp_world_size
            )
            if shard.local_end - shard.local_start != local_seq_len:
                raise ValueError(
                    f"local sequence length {local_seq_len} != local span "
                    f"{shard.local_span}"
                )
        else:
            cp_metadata = (
                PackedAttentionMetadata.from_cu_seqlens(
                    cu_seqlens.tolist(), local_seq_len
                )
                if cp_enabled
                else None
            )
        ...
        decoder_input, t_emb = self._embed(
            ...
            text_embed_select=text_embed_select,
            refiner_cu_seqlens=refiner_cu.to(device), refiner_max_seqlen=refiner_max,
            seq_len=local_seq_len, device=device,
            mem_pos=None if mem_pos is None else mem_pos.to(device),
        )
        ...
        # DeepSpeed CPU checkpointing empties saved floating tensors in place.
        # These tensors are shared by every DiT block, so keep them out of CPU
        # checkpoint storage and let them stay alive across the layer loop.
        t_emb.no_checkpointing = True
        rope_freqs.no_checkpointing = True
        for block in self.blocks:
