...
def kaiser_sinc_filter1d(cutoff, half_width, kernel_size):
    # DeepSpeed ZeRO-3 may change PyTorch's default device while constructing
    # modules.  Build this fixed filter explicitly on CPU so the window and
    # time grid cannot end up on different devices during initialization.
    ...
    if A > 50.0:
        ...
        beta = 0.0
    window = torch.kaiser_window(kernel_size, beta=beta, periodic=False, device=filter_device)
    if even:
        time = torch.arange(-half_size, half_size, device=filter_device) + 0.5
    else:
        time = torch.arange(kernel_size, device=filter_device) - half_size
    if cutoff == 0:
