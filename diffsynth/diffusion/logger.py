...
class ModelLogger:
    ...
    def on_step_end(self, accelerator: Accelerator, model: torch.nn.Module, save_steps=None, **kwargs):
        # Only advance on real optimizer steps. With gradient accumulation the
        # intermediate micro steps share one optimizer step; counting them here
        # would make step-N safetensors names and CSV rows drift from the
        # optimizer-step semantics used by full checkpoints and resume.
        ...
        if accelerator.is_main_process:
            ...
            components = kwargs.get("continuation_components")
            if components is not None:
                for logger in self.loggers:
                    for key in ("video_loss", "audio_loss"):
                        if key in components:
                            logger.log(key, components[key], self.num_steps)
